...
# Human sign-off on the extracted schema. Recorded here so the generated report
# and review page state it from one place. Any change to either source means
# re-running the extractor, which invalidates this -- the report prints the
# current source hashes so drift from the signed-off state is visible.
SIGNOFF = {"owner": "Alex Button", "date": "2026-08-05"}
...
def write_report(path, rows, orphans, templates, renames, conflicts, unparsed, checks, sources,
    ...
    L.append("Generated by `tools/extract_servtracker_schema.py`.\n")
    L.append("**Signed off by %s on %s.** (The CaseWorthy equivalent is Russ, who confirmed"
             % (SIGNOFF["owner"], SIGNOFF["date"]))
    L.append("`target_schema_full.json` on 2026-08-04.)\n")
    L.append("Sign-off covers the extracted schema. The findings below about the *source*")
    L.append("script -- checks that never run, wording that states no machine-readable rule --")
    L.append("are open items against that script, not against this schema.\n")
    L.append("Re-running the extractor after either source changes invalidates this sign-off:")
    L.append("compare the source hashes below against the ones recorded at sign-off.\n")
    L.append("## Sources\n")
